...
// s is an instance of Server, registerMessage is Server's function
func (s *Server) registerMessage(schema *schemabuilder.Schema) {
	...
	object.FieldFunc("reactions", func(ctx context.Context, m *Message) ([]*Reaction, error) {
		...
		// create the map of reactions ( ':(', ':)' )
		// ..where the value of the reaction key is a new Reaction object
		...
		// if err != nil after creating err via :=
		// &instances is like 'ref instances', where the value becomes modified by the function
...
// s is an instance of Server, registerQuery is Server's function
func (s *Server) registerQuery(schema *schemabuilder.Schema) {
	object := schema.Query()

	object.FieldFunc("messages", func(ctx context.Context) ([]*Message, error) {
		var result []*Message
		// &result is like 'ref result', where the value becomes modified by the function
...
// s is an instance of Server, registerMutation is Server's function
